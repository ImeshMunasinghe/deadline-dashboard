...
## Current State
- **Sri Lankan public holidays on the calendar** (2026-09-07): new pure module `src/utils/holidays.ts` with curated per-year datasets for 2025-2027 (all 12 Poya days, Vesak, Eid x2, Good Friday, Deepavali, Thai Pongal, New Year, National Day, May Day, Christmas — verified via officeholidays.com); uncovered years fall back to fixed-date holidays only (no guessed lunar dates). `getSriLankanHolidayMap(year)` is cached per year. `CalendarDayEvents` gains a `holidays` array; `collectCalendarEvents` takes an optional holiday map and merges holiday-only days. CalendarView shows rose-colored holiday chips on day cells (Moon icon for Poya, Sparkles for public holidays, excluded from +N overflow) and holiday rows in the day panel; legend includes "Holiday (LK)". 19 tests pass.
